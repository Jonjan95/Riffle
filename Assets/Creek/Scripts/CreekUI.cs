...
namespace RiffleCreek
{
    public sealed class CreekUI : MonoBehaviour
    {
        ...
        static readonly string[] upgradeNames={"Better pan","Better riffles","Better scoop"};
        static readonly string[] upgradeInfo={"+18% separation speed / level","+35% settling speed / level","+3 gold per scoop / level"};
        ...
        void OnGUI()
        {
            ...
            Label(555,43,215,20,"SCOOP "+sim.ScoopNumber.ToString("00"),small,muted);
            ...
            Box(new Rect(1258,194,310,375),cream,true);
            Label(1279,213,268,30,"A little better",heading);
            Label(1279,246,270,23,"Good tools. A steadier rhythm.",small,muted);
            for(int i=0;i<3;i++)
            {
                float y=285+i*90;
                Label(1279,y,180,25,upgradeNames[i],body);
                for(int k=0;k<3;k++)Box(new Rect(1279+k*14,y+30,8,5),k<p.Level(i)?teal:new Color(.76f,.76f,.66f));
                Label(1279,y+43,254,20,upgradeInfo[i],small,muted);
                bool max=p.Level(i)>=Progression.MaxLevel;
                if(Button(new Rect(1463,y-2,84,36),max?"MAX":p.Cost(i)+" g",p.CanBuy(i)))game.Buy(i);
            }
            Box(new Rect(1258,585,310,137),cream,true);
            Label(1279,602,262,22,p.SluiceActive?"THE CREEK IS WORKING":"FIRST MILESTONE",small,muted);
            Label(1279,630,267,30,p.SluiceActive?"A little sluice, alive":"Wake the little sluice",heading);
            Label(1279,664,262,23,p.SluiceActive?"Water wheel turning. Camp improved.":Mathf.Min(p.LifetimeGold,40)+" / 40 lifetime gold collected",small,muted);
            Bar(1279,702,267,p.LifetimeGold/40f,gold,5);
            ...
            Meter(49,614,"Loose",sim.Looseness,teal);
            ...
            WorldTag(new Vector3(-1.6f,1.3f,1.25f),"SMOOTH REAR  /  DEEP BOWL",false);
            WorldTag(new Vector3(-5.3f,.8f,-4.4f),sim.Ready?"GOLD READY  /  [E] COLLECT":"FRONT  /  WORKING RIFFLES",sim.Ready||game.Intent.Working);
            if(game.ToastTime>0)
            {
                Box(new Rect(415,141,610,44),teal,true);
                Label(432,152,582,28,game.Toast,body,cream);
            }
            ...
            Label(874,808,300,23,"HOLD LMB  work / shake",small,muted);
            Label(874,836,295,23,"HOLD SPACE / RMB  wash     E  collect",small,muted);
            if(Button(new Rect(1194,810,230,45),sim.Ready?"Collect gold  [E]":sim.Loaded?"Working scoop "+sim.ScoopNumber:"Scoop the creek  [E]",!sim.Loaded||sim.Ready,true)) {if(sim.Ready)game.Collect();else game.Scoop();}
            ...
            if(game.Paused)
            {
                Box(new Rect(0,0,1600,900),new Color(.1f,.18f,.17f,.6f));Box(new Rect(540,340,520,200),cream,true);
                Label(574,371,453,43,"A moment by the creek",title);Label(575,432,440,70,"Paused. Press Esc to return.\nM toggles sound. H opens the field guide.",body);
            }
